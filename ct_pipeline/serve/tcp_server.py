...
def handle_match_command(conn, ref_ply, ref_dir, mode, send, threshold, verbose):
...
def handle_send_command(conn, request, send, verbose):
    ...
    send_glb(conn, patient_id, send=send, verbose=verbose)
...
def handle_quest_connection(conn, addr, ref_ply, ref_dir, mode, send, threshold, verbose):
    ...
    try:
        ...
        if command == "match":
            handle_match_command(conn, ref_ply, ref_dir, mode, send, threshold, verbose)
        elif command == "send":
            handle_send_command(conn, request, send, verbose)
        else:
