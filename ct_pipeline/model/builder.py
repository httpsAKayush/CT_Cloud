...
def build_patient(patient_id, fmt, base_dir, mode="both", make_merged=False,
    ...
    if make_merged:
        if not (results["raw"] and results["union"]):
            missing = []
            if not results["raw"]:
                missing.append("raw")
            if not results["union"]:
                missing.append("union")
            raise ValueError(
                f"--merge requires both raw and union .glb — missing: {', '.join(missing)}. "
                f"Merged models are built ONLY from raw.glb + union.glb (no volume "
                f"recomputation), so both must be built first — use --mode both."
            )
        out_path = os.path.join(MERGED_MODEL_DIR, f"{patient_id}.glb")
        if os.path.exists(out_path) and not overwrite:
            if verbose:
